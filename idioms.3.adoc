...
The following is usage for all functions within the *idioms* library.
...
*die* [*-e* _ERRNO_] [_FORMAT_]

==== Options

*-e _ERRNO_*::
    Exit with _ERRNO_ as status. If unspecified, *die* exits successfully (0).

=== *error*

error - print an error (using stderr), appending a newline

==== Synopsis

*error* [*-d*] [*-e* _ERRNO_] _FORMAT_
...
*-e* _ERRNO_::
    Return _ERRNO_ after printing the error. If *-d* is given, exit with _ERRNO_ instead.

=== *note*

note - print a note (using printf(1)), appending a newline
...
call - print command to standard error, run it, `die` if it doesn't exit successfully
...
*-n*::
    Don't actually run the command, just print it. You may also set __idioms_call_dry_run_ to do
    the same.

=== *run*

run - call a command, but in a subshell

==== Options

See call's OPTIONS section.
...
*_idioms_call_dry_run*::
    Always run *call* with the *-n* (dry run) option set.
